...
write_png_flip :: #force_inline proc(filename: string, w, h: int, $comp: int, data: []u8, stride_in_bytes: int) -> int {
...
write_tga_flip :: #force_inline proc(filename: string, w, h: int, $comp: int, data: []u8) -> int {
	#assert(comp >= 0 && comp <= 4);
	for j in 0..h/2-1 {
		for i in 0..w-1 {
			k1 := comp*(j*w + i);
